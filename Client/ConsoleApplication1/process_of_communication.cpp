		...
		if (pro->CheckStatusEnable(SERVICE_ONLINE, enable))//查看服务器是否在线
		{
			if (enable == TRUE)//说明服务器在线
			{
				...
				if (pro->heartbeat_delay_ == 6)
				{
					...
					sprintf_s(buffer, "%s__%d行 与服务器重新连接", __FILE__, __LINE__, GetLastError());
	...
	else
	{
		...
		if (pro->CheckStatusEnable(CLIENT_ONLINE, enable))//查看客户端是否在线
		{
			if (enable == TRUE)//说明客户在线
			{
				...
				{
					char buffer[MAX_PATH] = { 0 };
					sprintf_s(buffer, "%s__%d行 与客户端重新连接", __FILE__, __LINE__, GetLastError());
					UnusualRecode(buffer);
				}
				pro->heartbeat_delay_ = 0;
...
handscake_wait_time_(0xFFFFFFFF)
{
	...
	lstrcpy(buffer, file_mapping_name);
	lstrcat(buffer, L"_client");
	client_event_lock_ = new CEventLock(buffer);//初始化客户端互斥对象
	lstrcpy(buffer, file_mapping_name);
	lstrcat(buffer, L"_service");
	service_event_lock_ = new CEventLock(buffer);//初始化服务器互斥对象
...
BOOL CProcessOfCommunication::ClientDispose(char *filepath, DWORD wait_time)//客户端的操作
{
	...
	if (result == WAIT_TIMEOUT)//等待超时
	{
		...
		char buffer[MAX_PATH] = { 0 };
		GetOnlineStatus(m_online_status);
		sprintf_s(buffer, "%s__%d行 多线程等待超时,线上状态为:%d", __FILE__, __LINE__, m_online_status);
		UnusualRecode(buffer);
		goto unlock;
	}
	result = 0;
		...
		&& m_online_status & CLIENT_UNLOCK && link_status == LINK_SUCCESS)
	{
		...
		if (!ClientWaitResultStatus())//客户端等待服务器相应
		{
			char buffer[MAX_PATH] = { 0 };
			GetOnlineStatus(m_online_status);
			sprintf_s(buffer, "%s__%d行 设置客户端为接收数据状态结束失败,线上状态为:%d", __FILE__, __LINE__, m_online_status);
			UnusualRecode(buffer);
			goto unlock;
		}
		...
		{
			Data *_data = m_filemapping_->GetSharedData();
			string filename = _data->communication_head_.client_to_server_.file_name_;
			_data->communication_head_.server_to_client_.result_ =
				Data::CommunicationData::ServerToClient::RESULT_UNDISPOSED;//处理结果状态为未处理
			...
			else//服务器处理完毕，成功返回,读取处理结果
			{
				printf("%s:",filename.c_str());
				switch (_data->communication_head_.server_to_client_.result_)
				{
				case Data::CommunicationData::ServerToClient::RESULT_ERROR:
					printf("处理错误\r\n"); break;
				case Data::CommunicationData::ServerToClient::RESULT_NORMAL:
					printf("正常图片\r\n"); break;
				case  Data::CommunicationData::ServerToClient::RESULT_SEX:
					printf("性感图片\r\n"); break;
				case Data::CommunicationData::ServerToClient::RESULT_PORNOGRAPHIC:
					printf("色情图片\r\n"); break;
				case Data::CommunicationData::ServerToClient::RESULT_UNDISPOSED:
					printf("未处理\r\n"); break;
				}
			}
		}
...
BOOL CProcessOfCommunication::ServiceDataDispose()
{
	...
	strcpy_s(des_path, "c:\\temp");
	strcpy_s(file_path, send_data_->communication_head_.client_to_server_.file_name_);
	PathStripPathA(file_path);
	PathAppendA(des_path, file_path);
	HANDLE fh;
	DWORD dwWritten;
	//printf("准备将数据写入%s中\r\n", des_path);
	fh = CreateFileA(des_path, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL);
	WriteFile(fh, (LPSTR)send_data_->communication_head_.client_to_server_.data_,
		send_data_->communication_head_.client_to_server_.data_size_, &dwWritten, NULL);
	CloseHandle(fh);
	return TRUE;
}
...
BOOL CProcessOfCommunication::ClientDataDispose(char *filepath)
{
	...
	send_data_->communication_head_.server_to_client_.result_ =
		Data::CommunicationData::ServerToClient::RESULT_UNDISPOSED;//设置为未处理状态
	filestr.close();
